...
public final class JDBCTemplate {
	...
	public static <T> List<T> query(String sql, PreparedStatementSetter setter, ResultSetCallBack<T> handler) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		...
		try {
			...
			// 假设提供了处理结果集的接口的实现类对象
			if (handler != null) {
				dataList = new ArrayList<T>();// 首先产生一个泛型集合
				// 循环记录集
				while (rs.next()) {
					// 处理结果集并把记录存到泛型集合中
	...
	//查询数据库返回的是单个值
	public static <T> T singleQuery(String sql, PreparedStatementSetter setter, ResultSetCallBack<T> handler) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		T data=null;
